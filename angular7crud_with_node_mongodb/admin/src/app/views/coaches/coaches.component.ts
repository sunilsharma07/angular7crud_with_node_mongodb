...
import { EditcoachComponent } from './editcoach/editcoach.component';
...
export class CoachesComponent implements OnInit {
  ...
  /*ngOnInit(): void {
    ...
    that.dtOptions = {
      ...
      columns: [{ data: 'id' }, { data: 'first_name' }, { data: 'created_at' }, { data: 'total_students' },{data:''} ]
    };
  }*/
  ...
  ngOnInit() 
  {
    ...
    this.getcoachlisting(this.offset, this.limit);
  }



  getcoachlisting(offset: number, limit: number, resetPagination: Boolean = false)
  {
      ...
      let filter = {};
        ...
        this.coachesService.getCategories(offset, limit, sort, filter).subscribe(response => 
          {
  ...
  pageChanged(event: any, resetPagination: Boolean = false): void 
  {
    ...
    this.getcoachlisting(this.offset, this.limit, resetPagination);
  }
  ...
  openadd_or_editModal(type,data = {}, index = 0)
  {
        if(type=='add')
        {
            const initialState = 
            {
              ...
              class: "modal-lg"
                ...
                this.bsModalRef = this.modalService.show(AddcoachComponent, 
                  {
              ...
              var record_edit = this.categoriesList[index];
                ...
                var custom_obj = {
                          email:record_edit.email,
                          first_name:record_edit.first_name,
                          last_name:record_edit.last_name,
                          status:record_edit.status,
                          phone:record_edit.phone,
                          image : record_edit.image,
                          _id:record_edit._id
                };
              ...
              this.bsModalRef = this.modalService.show(EditcoachComponent, 
                {
                      initialState, class: 'modal-lg', backdrop: true, ignoreBackdropClick: true
              });
        ...
        this.subscriptions = this.modalService.onHide.subscribe((reason: string) => {
          if (this.bsModalRef.content.type === 'success') {
                this.getcoachlisting(0, this.limit, true);              
          }
  ...
  updateStatus(status, categoryId, i, displayStatus)
    {
        ...
        Swal.fire({
          title: 'Are you sure?',
          text: 'You want to update coach status',
          type: 'warning',
        ...
        }).then((result) => 
        {
          ...
          {              
              ...
              let api_url = 'update_status_coach'; 

              this.coachesService.past_data_to_server(api_url,{ cate_id: categoryId,status: statusVal}).subscribe(response => {
                this.toastrService.success('Coach update successfully', 'Coach');
                this.getcoachlisting(this.offset, this.limit);
             }, error => {          
      ...
      }).then((result) => 
      {
        if (result.value) {
          this.toastrService.clear();
            ...
            let apiurl = 'delete_coach'; 

            this.coachesService.past_data_to_server(apiurl,{ cate_id: cateidd }).subscribe(response => {
              this.toastrService.success('coach delete successfully', 'Coach');
              this.getcoachlisting(0, this.limit, true);
